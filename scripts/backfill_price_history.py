...
def _ensure_sessionid() -> None:
    ...
    # (USD) запрошен явно. Заявление про sessionid было неверным. Настоящая
    # причина подтверждена отдельно (не в этом коде): валюта Steam-кошелька
    # аккаунта — рубли, и Steam на pricehistory всегда отдаёт цены в ней,
    # переопределяя любой запрошенный currency, — это свойство аккаунта, а
    # не то, что можно обойти какой-либо cookie. Сама функция оставлена —
    # дешёвая (один анонимный запрос) и, возможно, нужна для чего-то ещё на
    # pricehistory, что не проверялось.
